...
while True is True:
        ...
        if fluido_desejado in nomes_fluido: #Se o fluido desejado existe no dicionário:
            ...
            for fluido in fluidos: #acessando cada fluido do dicionário 'fluidos'
                if fluido_desejado == fluido['nome']: #Quando o fluido inserido for igual ao nome do fluido disponível no dicionário...
                    μ_float = fluido['μ'] #Aqui, definimos os valores das incognitas baseadas no fluido escolhido pelo usuário.
                    ρ_float = fluido['ρ'] 
                    

        else: #O fluido não existe no dicionário
            opcao = input('\nO fluido desejado não se encontra disponível na base de dados. Gostaria de personalizar as informações do seu fluido? [s]im ou [n]ão R:')
            opcao = opcao.lower()
            if opcao.startswith('s'): #o usuário quer inserir informações próprias para um fluido customizado...
                μ = input('\nInsira a viscosidade dinâmica desejada, em (Pa*s ou N*s/m²): ')
                ρ = input('\nInsira a massa específica desejada (em kg/m³): ')
                try: #tentando rodar o código sem erros...
                    μ_float = float(μ)
                    ρ_float = float(ρ)
                    
                except: #Se houver algum erro no código...
                    print('\nHouve um erro na inserção das características dos fluidos. Por favor, tente as inserir novamente:')
                    continue #o while é reiniciado.
            else: #se o usuário não quiser personalizar dados novos....
                continue #o while é reiniciado.
        ...
        Re = ρ_float*V_float*D_float/μ_float #Cálculo do número de reynolds.
